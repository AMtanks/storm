...
class SiliconFlowModel(dspy.OpenAI):
    """A wrapper class for SiliconFlow API, compatible with dspy.OpenAI."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.api_key:
            raise ValueError(
                "SiliconFlow API key must be provided either as an argument or as an environment variable SILICONFLOW_API_KEY"
            )

    def log_usage(self, response):
        """Log the total tokens from the SiliconFlow API response."""
        usage_data = response.get("usage")
        if usage_data:
            with self._token_usage_lock:
                self.prompt_tokens += usage_data.get("prompt_tokens", 0)
    ...
    def _create_completion(self, prompt: str, **kwargs):
        """Create a completion using the SiliconFlow API."""
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.api_key}",
        }
        ...
        # Ensure N is 1 if supplied
        if "n" in kwargs and kwargs["n"] != 1:
            raise ValueError("SiliconFlow API only supports N=1")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """Call the SiliconFlow API to generate completions."""
        assert only_completed, "for now"
        assert return_sorted is False, "for now"

        response = self._create_completion(prompt, **kwargs)

        # Log the token usage from the SiliconFlow API response.
        self.log_usage(response)
