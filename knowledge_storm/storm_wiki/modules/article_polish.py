...
class WriteLeadSection(dspy.Signature):
    ...
    **请始终使用中文编写，如文章中有英文片段，则翻译成中文，如遇专业术语或英文概念，可保留英文名词**。
    """
...
class PolishPage(dspy.Signature):
    """
    你是一位擅长查找文章中重复信息并删除它们的忠实文本编辑者，确保文章中没有重复内容。你不会删除文章中任何非重复的部分。你将适当地保留内联引用和文章结构（由"#"，"##"等标示）。
    请为以下文章执行这项任务。**如果文章中有英文片段，则翻译成中文，如遇专业术语或英文概念，可保留英文名词**。
    """
